...
function StatCard({ label, value, sub, color }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: '16px 20px', flex: 1, minWidth: 140 }}>
...
export default function Overview() {
  ...
  useEffect(() => {
    ...
    query('runs', { order: 'created_at', limit: 100 }).then(setRuns)
  ...
  // Use lambda_value (added in pipeline v2) or fall back to lambda_irma_2017
  const lastLambda = lastRun ? (lastRun.lambda_value ?? lastRun.lambda_irma_2017) : null
  // Avg surge index across runs that have a non-zero value
  const surgeRuns  = runs.filter(r => r.surge_index > 0)
  const avgSurge   = surgeRuns.length ? surgeRuns.reduce((a, r, _, arr) => a + r.surge_index / arr.length, 0) : 0
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        <StatCard label="LAST LAMBDA"     value={lastLambda != null ? lastLambda.toFixed(4) : '—'} sub={lastRun?.location || 'no runs yet'} color={C.accent} />
        <StatCard label="AVG SURGE INDEX" value={avgSurge > 0 ? avgSurge.toFixed(3) : '—'} sub="across completed runs" color={avgSurge > 1 ? C.err : avgSurge > 0 ? C.warn : C.muted} />
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                    {['Date', 'Location', 'Lambda', 'Surge', 'Status'].map(h => (
                ...
                <tbody>
                  {runs.slice(0, 10).map((r, i) => (
                    <tr key={r.id || i} style={{ borderBottom: `1px solid ${C.border}22` }}>
                      ...
                      <td style={{ padding: '6px 8px 6px 0', color: C.accent, fontWeight: 700 }}>{(r.lambda_value ?? r.lambda_irma_2017)?.toFixed(4) ?? '—'}</td>
                      <td style={{ padding: '6px 8px 6px 0', color: '#e2e8f0' }}>{r.surge_index > 0 ? r.surge_index.toFixed(3) : '—'}</td>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, padding: 20, marginTop: 20 }}>
        ...
        <div style={{ display: 'flex', gap: isMobile ? 20 : 40, flexWrap: 'wrap' }}>
          {[
            { label: 'Hurricane Irma 2017',    lambda: '0.0659', regime: 'RADIATIVE', precip: '254.3 mm' },
            { label: 'Hurricane Matthew 2016', lambda: '0.0401', regime: 'RADIATIVE', precip: '173.1 mm (peak)' },
          ].map(v => (
            <div key={v.label}>
